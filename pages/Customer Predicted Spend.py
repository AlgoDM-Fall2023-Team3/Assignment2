...
session.use_database("REG_SNOWPARK_ROI_DEMO")
session.use_schema("REG_AD_DATA")
...
with col3:
    ...
    bar_data = {
    'Label': ['Min Spend', 'Max Spend'],
    'Value': [minspend, maxspend]
}

    # Create a DataFrame from the bar_data dictionary
    bar_df = pd.DataFrame(bar_data)
    st.bar_chart(bar_df.set_index('Label'))
